...
private struct QuickActionsView: View {
  ...
  var body: some View {
    VStack(spacing: 12) {
      ...
      HStack(spacing: 16) {
        Button("Simulate Offer") {
          // Example action - simulate receiving an offer
          let mockOffer = WebRTCOffer(
            sdp: "mock-offer-sdp",
            type: "offer",
            clientId: "demo-user-\(Int.random(in: 1...999))",
            videoSource: "camera"
          )
          store.send(.view(.handleRemoteOffer(mockOffer, userId: mockOffer.clientId)))
        }
        .buttonStyle(.plain)
        .padding(.vertical, 8)
        ...
        Button("Disconnect All") {
          for peer in store.connectedPeers {
            store.send(.view(.disconnectPeer(userId: peer.id)))
          }
        }
        .buttonStyle(.plain)
        .padding(.vertical, 8)
